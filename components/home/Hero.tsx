...
import { ChevronLeft, ChevronRight, Utensils, Sparkles, Heart } from "lucide-react";
...
export default function Hero() {
  ...
  const slides = [
    {
      ...
      description: "Découvrez notre cuisine raffinée élaborée avec des produits frais et de saison."
    },
    {
      image: wingi18.src,
      alt: "Plat traditionnel africain",
      title: "Saveurs authentiques",
      description: "Des recettes traditionnelles revisitées avec une touche contemporaine."
    },
    {
      image: wingi2.src,
      alt: "Service traiteur événementiel",
      title: "Événements mémorables",
      description: "Service traiteur d'exception pour toutes vos occasions spéciales."
    }
  ];
  ...
  return (
    ...
    >
      ...
      {/* Conteneur des images avec animation améliorée */}
      <div className="absolute inset-0">
        {slides.map((slide, index) => (
          ...
          >
            ...
            {/* Légère animation de particules alimentaires */}
      ...
      {/* Contrôles de navigation stylisés */}
      <button
        onClick={prevSlide}
      ...
      <div className="container mx-auto px-4 relative z-10 text-center py-20">
        <div className={`transition-all duration-1000 ease-out ${
          ...
          <div className="inline-flex items-center bg-amber-500/10 backdrop-blur-sm px-4 py-2 rounded-full mb-6 border border-amber-400/30 animate-fade-in">
            <Heart className="h-4 w-4 text-amber-500 mr-2" fill="currentColor" />
            <span className="text-amber-300 text-sm font-medium">Nouveau menu saisonnier</span>
          </div>
          
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-serif font-bold text-white mb-6 tracking-tight leading-tight">
            <span className="block animate-fade-in-up delay-100">{slides[currentSlide].title}</span>
          </h1>
